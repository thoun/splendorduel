...
////////////
    ...
    public function loadBugSQL(int $reportId)
    {
      ...
      $sql = ['UPDATE global SET global_value=30 WHERE global_id=1 AND global_value=99'];
      $map = [];
      foreach ($players as $pId) {
        ...
        foreach ([1,2,3,4,5, 9] as $i) { $sql[] = "UPDATE card SET card_location='player$studioPlayer-$i' WHERE card_location='player$pId-$i'"; }
        $sql[] = "UPDATE royal_card SET card_location_arg=$studioPlayer WHERE card_location_arg = $pId";
        $sql[] = "UPDATE token SET card_location='player$studioPlayer' WHERE card_location='player$pId'";
